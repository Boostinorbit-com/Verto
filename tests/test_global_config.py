"""Global (machine-wide) config tier: `~/.config/verto/config.toml` layered UNDER the project
`.verto.toml`. Precedence: project > global > code defaults (the git model).
...
from verto.engine import config as _config
from verto.engine.config import Config, global_config_path
...
def test_global_config_path_is_xdg(tmp_path, monkeypatch):
    ...
    assert global_config_path() == tmp_path / "cfg" / "verto" / "config.toml"
...
def test_project_overrides_global(tmp_path):
    _write_global('model = "local"\nmin_rung = 1\n')
    proj = tmp_path / ".verto.toml"
    proj.write_text('[verto]\nmodel = "rules"\n', encoding="utf-8")   # project wins on model
