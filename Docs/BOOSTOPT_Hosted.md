# BOOSTOPT Hosted — the cloud version, explained from scratch

**This document explains roadmap item #19: "hosted / cloud" — a version where BOOSTOPT runs on *our* machines instead of yours, so you need no local setup.** It assumes no prior knowledge: every term is defined the first time it appears. It also says, honestly, when you *don't* need this.

---

## 1. What "hosted" means, and the one confusion to clear first

BOOSTOPT can run in three places. Don't mix them up:

| Where BOOSTOPT runs | What it is | Your code goes to |
|---|---|---|
| **Your laptop** (the CLI) | you run `boostopt` yourself | stays on your machine |
| **Your CI** (#18, the Action) | runs on *your* CI runners, in your pipeline | stays in your CI environment |
| **Our cloud** (#19, this doc) | runs on **BOOSTOPT's** machines | **sent to BOOSTOPT's servers** |

So "hosted" means: **you don't install or run anything; you point BOOSTOPT at your code and it does the compiling, sanitizing, and benchmarking on its own hardware.** The trade for that convenience — your code leaves your environment — is the whole discussion below (§5).
...
```
   You connect a repository (once)
            │
            ▼
   BOOSTOPT's cloud picks up each change and, on its OWN machines:
...
- **Hosted / Cloud / SaaS** — "Software as a Service." Instead of you running the software, a provider runs it for you and you use it over the internet. Gmail is SaaS; this is BOOSTOPT as SaaS.
...
- **On-prem / BYOC** — "on-premises" / "Bring Your Own Cloud" — **BOOSTOPT's software runs inside the customer's own datacenter or cloud account**, so their code never leaves *their* environment. The privacy-friendly option.
...
- **Flywheel** — a **self-reinforcing loop**: the more code BOOSTOPT optimizes, the more it learns which optimizations tend to work, so it gets better and faster for the next user. (More below — §7.)
...
2. On each change, BOOSTOPT **queues a job**.
3. A **worker** in the build farm picks it up, in an **isolated** environment.
4. The worker **compiles, differential-tests, runs sanitizers, and benchmarks** the candidate optimizations — the exact same gate as the CLI, just on BOOSTOPT's hardware.
5. Results are **returned to you** — as a PR/MR comment (same layout as #18) and/or on a **web dashboard**.
6. What BOOSTOPT *learned* (which optimization patterns worked) updates the shared **flywheel** — never your private code, just the patterns (§7).
...
This is the crux, and it must be said plainly. BOOSTOPT's defining promise everywhere else is *"runs on your machine; your source never leaves."* **A multi-tenant hosted service breaks that** — your code is sent to BOOSTOPT's servers to be compiled and benchmarked.

Three **deployment models** resolve this differently — a privacy ladder:

| Model | Where your code lives | Convenience | Privacy |
|---|---|---|---|
| **Multi-tenant SaaS** | on BOOSTOPT's shared servers | highest | lowest |
| **Single-tenant** | on a BOOSTOPT-run instance dedicated to you | high | medium |
| **On-prem / BYOC** | **in your own cloud/datacenter** | medium | **highest** |

**Recommendation: lead with on-prem / single-tenant.** It keeps BOOSTOPT's privacy advantage, defers the biggest liability (being custodian of everyone's private source), and it's the form enterprises — the buyers — prefer anyway. A public multi-tenant SaaS is the heaviest, most sensitive version; build it only if the market clearly pulls for it.
...
Building BOOSTOPT's own CI taught us something the hard way: **shared CI runners are noisy computers, and noisy computers give unreliable speed measurements.** A change that's genuinely 30% faster can measure as 1% faster on a busy runner, because other people's jobs are stealing the CPU. BOOSTOPT, being honest, then reports a smaller win — or none.

A **hosted build farm fixes this at the source**: BOOSTOPT controls the hardware, so it can run benchmarks on **quiet, dedicated, pinned machines** — a "clean room." That yields **numbers you can actually trust**, which a shared GitHub/GitLab runner *cannot* give.
...
So the shared cache is really a **growing library of proven optimization *patterns***, not a store of anyone's private results. That's still a genuine network effect — the more BOOSTOPT sees, the smarter its *proposing* gets — and it's honest.
...
3. **Managed compute** — BOOSTOPT's farm does the heavy compiling/benchmarking, instead of burning *your* CI minutes.
...
- **Privacy is the pivot** — how you handle "customer code on our servers" (via on-prem/single-tenant) determines whether hosting strengthens or *undermines* BOOSTOPT's core promise.

---

## 13. Quick answers to the obvious questions

- **Is my code safe if it's hosted?** In multi-tenant SaaS it lives on BOOSTOPT's servers (encrypted, isolated, but present). Prefer **on-prem/BYOC**, where your code never leaves your own cloud. That's the recommended default for anyone sensitive.
- **Do I still need `clang` installed?** No — the whole point of hosting is that BOOSTOPT's machines have the toolchain. You install nothing.
- **Is it cheaper than running it in my own CI?** Sometimes — BOOSTOPT's farm replaces your CI minutes with dedicated hardware, and the benchmarks are more reliable. Whether that's cheaper depends on your usage.
...
*Companion docs: `BOOSTOPT_CI_Action` (the #18 Action, closely related), `BOOSTOPT_Surfaces` (all surfaces), `BOOSTOPT_Roadmap` (where #19 sits, and why it's last). One fact, one home.*
