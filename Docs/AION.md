...
**AION (AI Optimizer Network)** — an AI system that makes existing programs measurably faster by reasoning about them the way a senior performance engineer does, and that **only keeps a change it has proven to be both correct and faster.**
...
2. [What AION is — precise definition](#2-what-aion-is)
3. [Why AION is required — proven, not asserted](#3-why-aion-is-required)
4. [Background: how C++ actually becomes machine code](#4-background-the-c-compilation-pipeline)
5. [Where AION sits in the compilation stages](#5-where-aion-sits)
...
12. [How AION extends: the seven axes](#12-extension-axes)
13. [v0 scope, dependencies, and build order](#13-v0)
14. [Prior art and exactly how AION differs](#14-prior-art)
...
<a name="3-why-aion-is-required"></a>
## 3. Why AION is required — proven, not asserted
...
The correct fix is a *source-level* change a human (or AION) makes: reserve the space first.
...
**That gap is the reason AION exists.** Today, closing it requires a scarce senior engineer with a profiler and a week.
AION automates that engineer's loop — safely.
...
To say precisely *where* AION operates, we need the real stages.
...
This is the first structured form and the primary thing AION reads.
For `build()`:
```
...
```
Everything AION needs to recognize "vector grown by push_back in a loop with no prior reserve" is right here, as facts — no fragile text parsing required.
...
**It is NOT where AION operates**, and §3 shows why: it cannot make semantic changes.
...
That runtime signal is the other half of what AION consumes.

---

<a name="5-where-aion-sits"></a>
## 5. Where AION sits in the compilation stages

AION does not live *inside* any single stage.
...
| **AION** | **Stage 0, guided by 2–3 + runtime** | **yes** | **yes** | **yes** |
...
AION is *not* alone here, and its differentiation lives in *how deeply* it verifies and *what* it targets, **not** in occupying the cell.
...
Everything in AION is swappable — the language, the model, the specific transform — except one rule, which is the project's identity:

> **AION never keeps a change that is not both provably CORRECT and measurably FASTER.**
...
The consequences are the reason AION is trustworthy:
...
- **New capability — prevention:** the same contracts, run in CI, stop new code from *reintroducing* a pattern AION already fixed. The optimizer becomes a regression *preventer*, not just a fixer.
...
```
┌──────────────────────────────────────────────────────────────────────┐
│ INTERFACE     CLI (aion analyze | optimize | report) · CI · IDE       │
├──────────────────────────────────────────────────────────────────────┤
...
For contrast, the same code through `-O3` alone (Stages 5–6, all 107 passes) leaves `_M_realloc_insert` in the final assembly — **0% recovered.** AION recovers 71%, verified.

---

<a name="12-extension-axes"></a>
## 12. How AION extends: the seven axes
...
| Interface | `aion analyze` / `aion optimize --apply` / `aion report` |
...
**Honest framing, up front.** AION's core loop — *an LLM proposes a rewrite, a trusted check verifies it is correct, a benchmark verifies it is faster, accept only if both hold* — is **not novel**.
...
AION must differentiate on *how deeply* it verifies and *what* it optimizes — never on the loop itself.
...
| System | Level / language | Correctness gate | Profile-guided? | What it leaves open for AION |
|---|---|---|---|---|
...
| **ECO** *(Google, 2025)* | source, **hyperscale production** | verify + **human code review** + production success (>99.5%) | **yes** — fleet data + a mined **"anti-pattern dictionary"** | closest to AION's Learning + Network *at scale*, but internal, human-in-loop, pattern-dictionary rather than formal contracts |
| **"Verified Learning for Compiler Optimization"** *(2025)* | **LLVM IR** (lazification case study) | **untrusted generator + trusted Alive2 formal check** | no | this is essentially AION's *trust split + formal rung* — but at **IR level** and one narrow transform, not C++ source semantics |
...
| Work | What it does | Different from AION because |
|---|---|---|
...
### 14.3 Where AION honestly differs — and the risk
...
So AION's honest claim is **integration, not invention.**
...
That bet is **defensible but unproven — explicitly not a first-mover claim.** The core loop is commoditized; AION lives or dies on the rigor and the layer.

### 14.4 Is AION's *architecture* different? Split the question honestly

**At the block-diagram level — no.** AION's fundamental shape — *untrusted LLM proposer → trusted "correct-and-faster" gate → loop* — is shared, not novel:

| Architectural element | AION | Codeflash | CompilerGPT | "Verified Learning" |
|---|---|---|---|---|
| Untrusted proposer + trusted gate + loop | ✅ | ✅ | ✅ | ✅ |
| **Trust split** (LLM proposes, external oracle decides) | ✅ | partial (tests) | partial | ✅ **exactly this** |

The "Verified Learning" paper describes *an untrusted generator + a trusted external formal verifier, correctness enforced as a runtime control layer* — AION's trust split, verbatim, as an architecture.
Claiming "my architecture is different" at this level does not survive scrutiny.

**At the *layering* level — yes, as a combination:**

| Distinctive layer | AION | The others |
|---|---|---|
...
**The honest sentence:** *AION's core architecture (proposer + trusted gate) is shared with the field, including a paper with the exact trust split.
...
| **Generating LLVM passes risks silent miscompilation.** | Deferred entirely. Near-term AION does source→source and lets LLVM's *already-verified* passes run. Novel-pass generation waits until a formal verifier (Alive2) is in the loop. (§6, §9) |
| **AION executes LLM-proposed code to verify it.** | Verification runs in a **sandbox** — resource limits, no network. (§9.4, design rule 6) |
...
- **AST (Abstract Syntax Tree):** the typed, structured form of your program after parsing (Stage 2–3); AION's primary source of static facts.
...
- **Differential testing:** running two versions on the same inputs and asserting identical output — Rung 1 of AION's correctness proof.
...
- **The invariant:** *never keep a change that is not both provably correct and measurably faster* — AION's identity.
