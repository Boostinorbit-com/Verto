# VERTO — Engineering Architecture (Generic / Multi-Language)
...
- *When a second language (Python) lands,* §16 splits into its own `VERTO_Architecture_Cpp` and each language gets a child doc — the split **earned at instance #2, not before** (the same rule-of-three discipline we apply to the code).

**Boundaries (one fact, one home):** concepts/why → `VERTO.md` · delivery → `VERTO_Surfaces` · proof → `WEDGE_TEST` · **how it's built → this doc** (C++ instance in §16).
...
VERTO is designed to optimize **any** language (Axis A: C++ → Python → Rust / Java / Go / JS). It does this **not** by cramming languages into one code path, but by keeping a small, fixed **engine** and swapping **thick, language-specific adapters** behind fixed interfaces.

This document specifies the fixed part and the interface every language plugs into.

```
                 ┌──────────────────────────────┐
                 │   VERTO.md   (concepts / why)  │
                 └──────────────┬───────────────┘
                                │
             ┌──────────────────┴───────────────────┐
             ▼                                       ▼
   VERTO_Architecture (this)                  VERTO_Surfaces (delivery)
...
│  ├─ config.py                   # .verto.toml, policy (min-rung, objectives…)
...
│
├─ surfaces/ …                     # thin clients (see VERTO_Surfaces)
└─ tests/
```
...
Responsibility  Load .verto.toml; hold policy (min-rung, objectives, budgets,
...
A working VERTO instance = one `LanguageAdapter` × one `DomainAdapter` × one `ModelProvider`, wired by the Registry.
...
- **Modest context** — VERTO feeds *source + compact facts*, not raw AST dumps, so a huge context window is not needed.
...
Runtime selection is the `--model NAME` / `--offline` flags (see `VERTO_Surfaces`).
...
**Why Python is #2 (not the biggest-market pick):** it's *maximally different* from C++ (cache-layout vs escape-the-interpreter). If the generic engine survives that jump, it survives anything — the best possible stress test of this architecture. (See `VERTO.md` §12.)
...
The one concrete instantiation of the generic architecture: the **C++ Language adapter** + **Performance Domain adapter** that make up v0. *(When a second language lands, this section splits out into its own `VERTO_Architecture_Cpp`.)*
...
8. **CLI surface** — `analyze` / `optimize` / `report` (see `VERTO_Surfaces`).

> Steps 2–3 are VERTO — and they are *also* the Wedge Test's judge, so building the core and building the proof are the same work.

---

*The engine is universal; languages are adapters. This doc owns the universal part, the adapter contracts, and (today) the concrete C++ instance in §16. When a second language lands, §16 splits into `VERTO_Architecture_Cpp` and each language gets its own child doc. Concepts → `VERTO.md` · delivery → `VERTO_Surfaces` · proof → `WEDGE_TEST`.*
