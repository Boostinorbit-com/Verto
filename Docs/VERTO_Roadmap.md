...
> - **B2-a — Pointer+length *synthesis* (#2) — DO FIRST.** `f(const T* p, size_t n)` reads `p[0..n)` — the length is *in the signature*, so `gather` doesn't actually need *capture*, it needs *synthesis*. Statically **prove** `p` is read-only within `[0, n)` (no `p[n]`/`p[-1]`/writes/escape), then synthesize a `T[n]` buffer with the existing fuzz heuristic. **Sound** (same buffer feeds orig + variant; ASan backstops a mis-inferred range) and needs **no recording infrastructure** — the cheap road to the `gather(const int* src, size_t n)` skip. Not provable → honest skip (or fall through to B2-b). *Proof-point: flips `gather` SKIP → ACCEPT on `examples/linked/report.cpp`.*
...
>
> **Build order:** **B2-a** (covers `gather` cheaply + soundly, high proof value) → **B2-b** STL-value types → **B2-b** raw pointers → **C**. **Measure-first before B2-b:** confirm real-pointer *capture* (not STL-value args or aggregates) is the dominant skip on real repos before building the recorder. **Sound-by-default throughout:** synthesize/capture only what's *provably* safe to read; otherwise honest skip.
>
