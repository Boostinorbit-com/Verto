...
- Input coverage for custom types is **synthesis only** (aggregate structs of primitive fields, and `const T*`+length buffers — B2-a); **capturing real argument values** from a run is future work (#2-B2-b).
...
2. **Capture & replay** ⏳ **v0 slices done (aggregate synthesis + pointer+length synthesis)** — VERTO now **synthesizes inputs** for (a) functions taking a **simple aggregate struct** (all public primitive fields; no user ctor / base / virtual) by brace-initializing it field-by-field, and (b) **`const T*` pointer + length** pairs (B2-a) by inferring the safe read range `[0, n)` from the `(ptr, len)` convention and fabricating a length-`n` buffer. Sound for the differential test (the *same* synthesized input feeds orig + variant), with a size-vs-value heuristic so count fields stay large (reserve win stays measurable) and signed fields stay small (no overflow UB — which UBSan would rightly reject); for pointers, `const` gives read-only-by-type and ASan backstops a mis-inferred length. Proven: `scaled_series(const Config&)` flipped **SKIP → ACCEPT (−68%)**, and `gather(const int* src, size_t n)` — a raw pointer that used to be an honest skip — now flips **SKIP → ACCEPT** (`out.reserve(n)`); a **non-const** `int*` (`mix`) correctly stays an honest skip (can't prove read-only). *Remaining tiers (see the box below): **B2-b** real-value **capture** (record real args from a workload run) for what synthesis can't prove, then **C** full capture.* — `analysis/pointers.py` (`pointer_length_pairs`), `harness/synth.py` (`_ptr_builder`), `harness/template.py`, `analysis/types.py` (`aggregate_fields`); example `examples/linked/report.cpp`
...
> - **B2-a — Pointer+length *synthesis* (#2) ✅ done (v0).** `f(const T* p, size_t n)` reads `p[0..n)` — the length is *in the signature*, so `gather` doesn't need *capture*, it needs *synthesis*. Implemented soundly by **type**, not deep analysis: accept only a **`const T*`** (T primitive) that is **indexed** (`p[...]`) and paired with a sibling integer length; synthesize a length-`n` buffer with the existing fuzz heuristic (the *same* buffer feeds orig + variant). Soundness: `const` ⇒ read-only by type (can't corrupt caller state); a mis-inferred length can only cause an out-of-bounds **read**, which ASan traps on **both** orig + variant → no clean run → honest skip, never a false accept. A **non-const** `int*` is deliberately *not* synthesized (can't prove read-only) → honest skip → that's B2-b's job. Needs **no recording infrastructure**. *Proven: `gather(const int* src, size_t n)` flips **SKIP → ACCEPT** (`out.reserve(n)`) on `examples/linked/report.cpp`; `mix(int* src, size_t n)` stays an honest skip.* — `analysis/pointers.py`, `harness/synth.py` (`_ptr_builder`), `harness/template.py`.
> - **B2-b — Capture real values from a run (#2).** For pointers whose length can't be inferred, handles, or invariant-bearing types: a codegen **recording shim** logs the real argument bytes during the user's workload — a new **`--capture-command`** (sibling of `--test-command`/`--bench-command`) — then a **replay harness** reconstructs and differential-tests them. **Serialization is staged by type** (C++ has no reflection): **STL value types first** (`std::vector`/`std::string`/`std::span` carry their own size → easier than raw pointers) → raw pointers → aggregates-of-aggregates. Coverage is workload-bounded, so **fuzz *around* captured values** to widen it, and disclose *"verified on N captured + M fuzzed inputs."* Capture yields **inputs only** — the original stays the oracle (no oracle-in-the-gate).
> - **C — Capture & replay, full (#2).** Recorders for arbitrary types (pointer graphs, resources, deep object state). The real prize, and a multi-week research effort on its own.
>
> **Build order:** **B2-a** ✅ (covered `gather` cheaply + soundly, high proof value) → **B2-b** STL-value types → **B2-b** raw pointers → **C**. **Measure-first before B2-b:** confirm real-pointer *capture* (not STL-value args or aggregates) is the dominant skip on real repos before building the recorder. **Sound-by-default throughout:** synthesize/capture only what's *provably* safe to read; otherwise honest skip.
>
...
16. **Packaging** ✅ **done (v0) — 2026-07-26; publish on hold (proprietary, 2026-07-27).** Package builds as **`verto-optimizer`** (`verto` was taken; import + CLI stay `verto`). **License is now Proprietary / all rights reserved** (was Apache-2.0 — changed 2026-07-27 to keep every future option open while pre-launch; `LICENSE`/`NOTICE` are proprietary notices, `pyproject.toml` carries `Private :: Do Not Upload` so PyPI **rejects** any upload). Full `pyproject.toml` metadata (classifiers, keywords, URLs, extras: `dev`/`llm-openai`/`llm-anthropic`/`keychain`). **Wheel builds cleanly** (`verto_optimizer-0.1.0-py3-none-any.whl`, entry point verified). Zero-setup **`Dockerfile`** (bundles clang+sanitizers) + `.dockerignore`. `RELEASING.md` documents the intended build→PyPI flow **but is gated** until an open-source-license decision is made. *⏳ deferred: pick the eventual license split (open-core: Apache/MIT on the core, proprietary premium) at open-source time, then publish.* — `pyproject.toml`, `LICENSE`, `NOTICE`, `Dockerfile`, `RELEASING.md`
...
18. **CI / GitHub Action** ✅ **DONE (v0) — verified on a live PR 2026-07-27.** Runs on every pull request and comments the verified findings. *The paid surface.* **Explained from scratch (every concept) in [`VERTO_CI_Action.md`](VERTO_CI_Action.md) · [html](VERTO_CI_Action.html).** All four build steps (below) shipped, then **proven end-to-end on a real PR** (`Boostinorbit-com/Verto`, branch `verto-ci-action`): the `verto-demo.yml` workflow ran the Action's `entrypoint.py` on the bundled demo project and posted the verified `reserve()` finding as a PR comment. *(Deferred, not blocking: `mode: pr` auto-PR, `fail-on: regression`, and publishing the image to GHCR so OTHER repos can `uses:` it — the live proof ran the entrypoint directly.)*
    ...
    - **✅ End-to-end proof done (2026-07-27):** opened a PR on `Boostinorbit-com/Verto` from branch `verto-ci-action`; `verto-demo.yml` (runs `entrypoint.py` directly — no image needed) executed the Action and **posted the verified finding as a PR comment.** Confirms the whole chain: inputs → CLI → gate → outputs → exit code → comment.
    - **Deferred (not blocking v0):** ① publish the image to GHCR so *other* repos can `uses: Boostinorbit-com/Verto@v1` (the live proof ran the entrypoint directly); ② `mode: pr` auto-PR (logs + falls back to summary+suggestions today); ③ `fail-on: regression` (needs the baseline-diff feature).
