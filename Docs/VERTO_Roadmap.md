...
**2A. Oracle reach via test-reuse** ✅ **done (v0)** — *the keystone; removes the signature ceiling.* **Proven:** `scaled(const std::map<int,long>&, int)` — a function the synth harness **can't** build an oracle for — flips **SKIP → ACCEPT (−20%)**, verified by the project's OWN test (correctness) + bench (perf) via `--test-command`/`--bench-command` (`examples/reach/`). *v0 scope: routes a signature-unharnessable func to test-mode when no harnessable candidate exists; the bench times the whole `--bench-command` (build included → make the workload dominate) and isn't core-pinned — a coarse-but-sound Pareto signal. Side-effect (1c) funcs via tests, and multi-candidate test-mode selection, are follow-ons.*
