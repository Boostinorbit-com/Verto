...
15. **Tests + CI** ◑ **CI shipped (v0) — 2026-07-25.** The **test suite already existed** (125 tests + the 14-case wedge); this item was really *CI*. `.github/workflows/ci.yml` runs `pytest` + `wedge` on every push/PR, matrix **Python 3.11 / 3.12**, installs `clang`+sanitizers+`bubblewrap`+`ccache` (cached), live-LLM tests stay opt-in (no Ollama in CI), sandbox tests `skipif`-degrade where isolation is unavailable. **The one real risk — benchmark determinism on noisy shared runners** (we saw CPU contention flip genuine wins this session) — is handled pragmatically with `pytest-rerunfailures` (`--reruns 2`) + a wedge retry loop: a real regression fails all attempts, transient noise clears on retry. Locally certified green (125 pass, wedge 14/14). *⏳ remaining: first live GitHub run to validate the noise strategy on real runners; and the deeper fix — **interleaved A/B/A/B measurement** in `bench_runner` (baseline & variant currently timed at different moments → time-varying load biases the delta), which would harden perf robustness for CI **and** real users on loaded machines. — `.github/workflows/ci.yml`, `pyproject.toml` (dev extra)*
