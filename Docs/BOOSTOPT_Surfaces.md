# BOOSTOPT — Surfaces & Delivery

**How BOOSTOPT is delivered to a user: the CLI, the CI action, the IDE extension, and beyond.**

This document owns the *surface specs* — what each front-end looks like and does.
It does **not** own the Engine API those surfaces call; that lives in `BOOSTOPT_Architecture` (the "how it's built" doc).
For the *concepts* behind the work each surface triggers, see `BOOSTOPT.md`.

> **Scope note:** v0 targets **C++**. Multiple languages (Python → Rust / Java / Go / JS) are **Axis A** in `BOOSTOPT.md` §12 — planned and architected for, but not yet built. Everything below is C++ today.
...
```
   CLI        VS Code ext      CI action      Web dashboard      SDK
    ...
    └─────────────┴───────┬───────┴────────────────┴──────────────┘
                          ▼
             Engine API  (analyze · optimize · report)     ← spec'd in BOOSTOPT_Architecture
                          ▼
...
|---|---|---|---|
| **CLI** (`boostopt analyze / optimize --apply / report`) | terminal tool | **v0** | simplest; proves the engine end-to-end |
...
```
boostopt init     [--model NAME] [--pull] [--global]
    Set up the .boostopt/ performance workspace (like `git init`): ledger, baselines,
    cache, a pointer to the local model; auto-.gitignores it; writes a starter
    .boostopt.toml (committed team config, local-first). Detects the local model
    (Ollama) and reports readiness; --pull fetches it, --global also scaffolds
    machine-wide defaults at ~/.config/boostopt/config.toml. Idempotent.

boostopt analyze  <path> [-p DB] [--all] [--min-rung N] [--offline] [--diff] [--json]
    Non-destructive. Detect hotspots + candidate transforms and explain them.
    Writes nothing. The "what would you do?" command.

boostopt optimize <path> [-p DB] [--all] [--min-rung N] [--fast] [--offline] [--json] [--apply]
...
**`.boostopt/` — the per-project workspace** (`boostopt init`, "git for performance"). Lives in the repo like `.git/`: the `ledger.jsonl` (every accept/reject), `baselines/` (the regression floor), `cache/`, and a `model` pointer. **Local & git-ignored** — generated state, never committed. The model *weights* never live here; they stay once in Ollama's global store (`~/.ollama`).

**Config layering** (the git model — project overrides user overrides defaults):

| Source | Scope | Committed? |
|---|---|---|
| `.boostopt.toml` (repo root) | project / team config | ✅ yes — shared |
| `~/.config/boostopt/config.toml` (XDG) | machine-wide user defaults (`boostopt init --global`) | per-machine |
| code defaults | built-in | — |

Precedence: **project `.boostopt.toml` > global `~/.config/boostopt/config.toml` > code defaults.** XDG (not `~/.boostopt/`) keeps the global config from ever colliding with the per-project `.boostopt/` that discovery walks up to find.

### Conventions

BOOSTOPT uses **GNU/POSIX-style flags as the base, with LLVM-friendly affordances** — familiar to `git`/`cargo` users *and* to `clang-tidy` users. (Rationale: BOOSTOPT has subcommands and a Python engine, where `llvm::cl`'s single-dash-long style fights the grain; but the audience is LLVM-native, so we adopt its habits where they help.)
...
**Raw compiler flags pass through after `--`** — the clean way to stay LLVM-native without polluting BOOSTOPT's own namespace:

```
boostopt optimize hist.cpp -p build/ --min-rung 3 -- -O3 -std=c++20 -Iinclude -DNDEBUG
                                                └──────── verbatim to clang ────────┘
```
...
| BOOSTOPT | clang-tidy analog | shared idea |
|---|---|---|
...
| `--config-file .boostopt.toml` | `--config-file .clang-tidy` | project config |
| `--format` | `--format-style` | reformat after edit |
| `--quiet` | `--quiet` | suppress noise |

### Flags

Grouped by concern. **Status:** ✅ wired (v0 CLI flag) · ⚙️ config-only (settable in `.boostopt.toml` today, no CLI flag yet) · ⚠️ flag present but not yet functional · **v1** / **later** planned. `BOOSTOPT_Flags.md` (generated from the parser) is the exact ✅ set; this table is the fuller roadmap. Each flag maps to a parameter of the Engine API (spec'd in `BOOSTOPT_Architecture`).
...
> environment variable** — `OPENAI_API_KEY` (or `BOOSTOPT_LLM_API_KEY`), **never a flag** (a key in
> `argv` leaks into `ps`/shell history):
> ```bash
> export OPENAI_API_KEY=sk-...
> boostopt optimize foo.cpp --model frontier \
...
**Correctness rigor** *(BOOSTOPT-specific — this is the differentiator)*
...
| Flag | Meaning | Stage |
|---|---|---|
| `--config-file .boostopt.toml` | reproducible config | ✅ |
| `--no-daemon` | run in-process, ignore a warm `boostopt serve` daemon | ✅ |
| `-V, --version` | print the BOOSTOPT version | ✅ |
...
Machine (`--json`): an array of `Verdict` objects (schema owned by `BOOSTOPT_Architecture`).
...
```
$ boostopt optimize packet_stats.cpp --profile perf.data
...
Runs BOOSTOPT automatically on every pull request, so optimization becomes part of code review instead of a separate chore.

**How it works, step by step:**
1. The PR triggers the action.
2. It runs `boostopt optimize --changed --base <target-branch> -p compile_commands.json --json` — scoped to just the files the PR touched.
...
- **`prevent` (blocking):** runs the **contracts as checks**. If new code re-introduces a pattern BOOSTOPT already fixed — e.g. a hot `push_back` loop with no `reserve` — the check **fails the build** (`--fail-on contract-violation`). This turns the optimizer into a regression *preventer*, the stickiest use.

**What the developer sees** — a bot comment on the PR:

```
🟢 BOOSTOPT — verified optimization in build_histogram()
...
**Tech:** a thin YAML action wrapping the CLI — no engine logic; it just runs `boostopt` and renders `--json`.
...
```yaml
# .github/workflows/boostopt.yml (sketch)
- uses: boostopt/optimize-action@v1
  with:
    paths: ${{ changed_files }}
...
Brings verified suggestions into the editor, where developers already work. **Full design note (every concept, the CodeLens/proof-on-hover/Apply UX, the honest-latency model): [`BOOSTOPT_VSCode.md`](BOOSTOPT_VSCode.md) · [html](BOOSTOPT_VSCode.html).**

**The experience, end to end:**
1. You open (or save) a C++ file.
2. In the background, BOOSTOPT analyses it and marks hot regions with a **gutter icon**.
...
- It talks to a **local `boostopt` daemon** over an LSP-style JSON-RPC protocol.
...
**Config:** reads the project's `.boostopt.toml` (min-rung, transforms, profile source), so editor and CLI behave identically.
...
The backend for **Axis E** — a shared store of verified `(transform, contract, rung)` tuples that makes every BOOSTOPT instance smarter.
...
The engine, importable — for when you want BOOSTOPT inside your own program instead of behind a CLI.
...
- **One Ledger.** Every accept/reject from every surface is recorded once, so `boostopt report` and the web dashboard always agree — there's no per-surface bookkeeping to drift.
- **One config.** `.boostopt.toml` is the single source for profile source, min-rung, budgets, and enabled transforms — change it once and every surface follows.
...
*Surface specs live here; the Engine API they all call is specified in `BOOSTOPT_Architecture`; the concepts behind the work are in `BOOSTOPT.md`. One fact, one home.*
