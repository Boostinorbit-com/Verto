# VERTO in Your CI — the GitHub Action, explained from scratch

**This document explains roadmap item #18: the VERTO GitHub Action — the thing a C++ team installs so that, on every code change, VERTO automatically finds verified speed-ups and shows them right in the pull request.** It assumes no prior knowledge: every term is defined the first time it appears.

---

## 1. Who this is for, and the one confusion to clear first

There are **two completely different things** in VERTO's plan that both contain the words "CI". Do not mix them up:

| | **VERTO's own CI** (#15, already built) | **The VERTO Action** (#18, this doc) |
|---|---|---|
| Whose project it runs in | The **VERTO repository** | **Your** C++ project |
| What it does | Tests that VERTO itself isn't broken | Runs VERTO **on your code** and suggests optimizations |
| Who benefits | VERTO's developers | **You and your team** |
...
```
   You open a Pull Request (a proposed code change)
            │
            ▼
   GitHub runs the VERTO Action automatically
            │
            ▼
   VERTO looks only at the files you changed, and for each one:
     ...
     · benchmarks it          (is the optimization actually faster?)
            │
            ▼
   VERTO writes a comment on your Pull Request:
     "Found 2 verified optimizations — here are the diffs. Click to apply."
```

You never leave GitHub. You never run anything by hand. VERTO does the work and *proves* each suggestion before showing it to you.
...
- **Pull Request (PR)** — a *proposed* change to a codebase, opened for review before it's merged in. It's where teammates comment, and where automated checks report. This is where VERTO puts its findings.
- **GitHub Action** — a **reusable piece of automation** you plug into a project. You add a small file and, from then on, GitHub runs that automation on the events you choose (e.g. "on every PR"). The VERTO Action is one of these.
...
- **`compile_commands.json`** — a file that lists **exactly how each of your C++ files is compiled** (which flags, include paths, defines). Real C++ can't be compiled without this information, so VERTO needs it. Most build systems can generate it automatically (CMake does it with one flag).
- **"Only the changed files" (`--changed`)** — instead of scanning your whole project every time (slow), VERTO looks **only at the files this PR touched.** Fast, and relevant.
- **A "verified finding"** — an optimization VERTO has **proven** is (a) behavior-identical and (b) faster. VERTO never shows you an unproven guess.
- **A GitHub "suggested change"** — a special kind of PR comment that contains a diff **you can apply with one click** in the review screen. VERTO can post its fixes this way.
...
4. **Run VERTO on just those files** — `verto optimize --changed <base> -p compile_commands.json`. For each candidate optimization, the trusted gate compiles it, differential-tests it, runs sanitizers, and benchmarks it.
...
Everything above is a thin wrapper over the same VERTO engine the command-line tool already uses — the Action just *triggers* it and *renders* the result into GitHub.
...
4. **Prevent mode** — **fail the check when VERTO found a verified win you didn't take** (`fail-on: any`). Turns the comment from advice into a gate: *"there's a proven, correct-and-faster change here — don't merge past it."* **Shipped.**
...
VERTO needs a C++ toolchain (`clang++` with sanitizers) to do its job. Rather than make every user install that, the Action ships as a **Docker action**: a pre-built container image that already contains clang, the sanitizers, and VERTO. GitHub pulls the image and runs it. The user installs *nothing* — they just add the workflow file.

(VERTO already has the `Dockerfile` for this; the Action is a thin `action.yml` on top of it.)

### CI portability — works on GitLab too (and any CI)

Because VERTO is really just **a CLI in a Docker image**, it runs on *any* CI — GitHub, GitLab, Jenkins, CircleCI. The engine and every "entry" are universal; only a thin *wrapper* per platform differs:

```
        one engine  (verto CLI + Docker)  ← universal; every input maps to a CLI flag
         ├── GitHub Action     (action.yml + PR comments)   ← GitHub wrapper
         └── GitLab Component  (spec:inputs + MR notes)      ← GitLab wrapper
```

**GitLab** has its own analog of the Action's inputs — a **CI/CD Component** with a `spec: inputs:` block. The *entries are the same* (they map to the same `verto` flags); only two things differ syntactically:
...
**A "runner" is a shared, rented computer, and shared computers are *noisy*** — other people's jobs are running on the same hardware, the CPU speed fluctuates, and timing measurements bounce around. VERTO's whole promise is *"correct AND faster,"* and *"faster"* is measured by timing. On a noisy runner, a change that's genuinely **30% faster on your laptop can measure as barely 1% faster on the runner** — and VERTO, being honest, would then say *"not faster, no finding."*

So the naive version of the Action would *under-report* real wins on noisy hardware and look weak.

**The key realization (learned the hard way building #15):** the two halves of VERTO's guarantee behave very differently on shared hardware —
...
The upshot: on CI, VERTO's unique, durable value is the **correctness proof and the non-obvious optimization** — the exact percentage is secondary. That's actually a *stronger* and more honest pitch than a fragile hard number.
...
**2. Add a workflow file** — `.github/workflows/verto.yml` (sketch):
```yaml
name: VERTO
on: pull_request
jobs:
  optimize:
    ...
    steps:
      - uses: actions/checkout@v4
        with: { fetch-depth: 0 }        # so VERTO can see the diff vs the base branch
      - run: cmake -S . -B build -DCMAKE_EXPORT_COMPILE_COMMANDS=ON
      - uses: verto/action@v1            # ← the VERTO Action
        with:
          compile-commands: build/compile_commands.json
          mode: comment                  # comment | suggest | pr
```

**3. (Optional) A `.verto.toml`** in your repo root sets team defaults once — which model, minimum speed-up, budget — and every run follows it.
...
An Action is defined by an **`action.yml`** file, which declares the **inputs** a user may pass (the `with:` keys), the **outputs** later steps can read, and how it runs. Here is the proposed contract — each input maps to a VERTO CLI flag that already exists, so this is a real spec, not a wish.
...
| `config-file` | path to the team's `.verto.toml` | `.verto.toml` | `--config-file` |
...
runs:
  using: "docker"            # reuses VERTO's Dockerfile / a published image
  image: "docker://ghcr.io/verto/action:v1"
  # the entrypoint reads inputs from INPUT_* env vars, runs `verto`, posts results
```
...
```yaml
      ...
      - run: echo "VERTO found ${{ steps.verto.outputs.findings }} optimizations"
```

**Copy-pasteable sample files** live in [`examples/github-action/`](../examples/github-action/): a fully-commented **`verto.yml`** (the workflow you drop into your repo) and the **`action.yml`** (this interface definition, as a real file).
...
- **Is this the same as VERTO's own CI?** No — see §1. That one tests VERTO; this one runs VERTO on *your* code.

---

*Companion docs: `VERTO_Surfaces` (all delivery surfaces), `VERTO.md` (the concept), `VERTO_Roadmap` (where #18 sits). One fact, one home.*
