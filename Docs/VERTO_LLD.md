# VERTO — Low-Level Design (LLD)

**A code-mapped walkthrough of how VERTO is actually built — read this alongside the source to navigate the ~5,250-LOC `verto/` package.**

Companion to `VERTO.md` (concept — *why*), `VERTO_Architecture.md` (blueprint — *what*), `VERTO_Surfaces.md`/`VERTO_Flags.md` (the CLI), `VERTO_Roadmap.md` (state & plan). **This doc is the *how* — the file/class/function map.** Every reference is `path — symbol`, so you can jump straight to the code.

---

## 1. The 60-second mental model

VERTO takes one function, proposes a change, and **accepts it only if a trusted gate proves it is byte-identical in behavior AND measurably faster.** Everything is organized around one invariant, enforced in exactly one place (`engine/gate.py — InvariantGate.decide`):
...
## 4. End-to-end trace: `verto optimize foo.cpp`
...
| `engine/workspace.py` | the `.verto/` per-project workspace (`verto init`): `find()`/`ledger_path()` (git-style walk-up), `init()`, `gitignore_add()`, `write_starter_config()` (root `.verto.toml`), `write_global_config()` (XDG). Model *pointer* only — weights stay in Ollama |
...
| `engine/config.py — Config` | the gate policy + all knobs; `Config.load()` **layers** global `~/.config/verto/config.toml` (XDG) under the project `.verto.toml` (precedence: project > global > defaults) |
...
*This LLD maps the code as built; the *why* lives in `VERTO.md`, the *what* in `VERTO_Architecture.md`. When the code and this doc disagree, the code wins — grep the symbol and update this map.*
