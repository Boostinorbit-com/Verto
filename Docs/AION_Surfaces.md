# AION — Surfaces & Delivery

**How AION is delivered to a user: the CLI, the CI action, the IDE extension, and beyond.**

This document owns the *surface specs* — what each front-end looks like and does.
It does **not** own the Engine API those surfaces call; that lives in `AION_Architecture` (the "how it's built" doc).
For the *concepts* behind the work each surface triggers, see `AION.md`.

> **Scope note:** v0 targets **C++**. Multiple languages (Python → Rust / Java / Go / JS) are **Axis A** in `AION.md` §12 — planned and architected for, but not yet built. Everything below is C++ today.
...
```
   CLI        VS Code ext      CI action      Web dashboard      SDK
    ...
    └─────────────┴───────┬───────┴────────────────┴──────────────┘
                          ▼
             Engine API  (analyze · optimize · report)     ← spec'd in AION_Architecture
                          ▼
...
|---|---|---|---|
| **CLI** (`aion analyze / optimize --apply / report`) | terminal tool | **v0** | simplest; proves the engine end-to-end |
...
```
aion analyze  <path> [-p compile_commands.json] [--profile FILE] [--json]
    Non-destructive. Detect hotspots + candidate transforms and explain them.
    Writes nothing. The "what would you do?" command.

aion optimize <path> [-p compile_commands.json] [--apply] [--min-rung N] [--profile FILE] [--json]
...
aion report   [--since DATE] [--json]
    Read the Ledger; show accepted/rejected episodes, rungs, and measured gains.

# -p / --compile-commands is REQUIRED for C++ — AION cannot compile a file without its
# exact build flags. Generate it via CMake: -DCMAKE_EXPORT_COMPILE_COMMANDS=ON.
```

### Conventions

AION uses **GNU/POSIX-style flags as the base, with LLVM-friendly affordances** — familiar to `git`/`cargo` users *and* to `clang-tidy` users. (Rationale: AION has subcommands and a Python engine, where `llvm::cl`'s single-dash-long style fights the grain; but the audience is LLVM-native, so we adopt its habits where they help.)
...
**Raw compiler flags pass through after `--`** — the clean way to stay LLVM-native without polluting AION's own namespace:

```
aion optimize hist.cpp -p build/ --min-rung 3 -- -O3 -std=c++20 -Iinclude -DNDEBUG
                                                └──────── verbatim to clang ────────┘
```

**Flag names mirror `clang-tidy`** where they overlap, so muscle memory transfers:

| AION | clang-tidy analog | shared idea |
|---|---|---|
...
| `--config-file .aion.toml` | `--config-file .clang-tidy` | project config |
| `--format` | `--format-style` | reformat after edit |
| `--quiet` | `--quiet` | suppress noise |

### Flags

Grouped by concern. **✅ = v0**; the rest are planned (v1/later) but pre-designed so they slot in without a redesign. Each flag maps to a parameter of the Engine API (spec'd in `AION_Architecture`).
...
**Correctness rigor** *(AION-specific — this is the differentiator)*
...
| Flag | Meaning | Stage |
|---|---|---|
| `--config-file .aion.toml` / `--config KEY=VAL` | reproducible config | ✅ |
...
Machine (`--json`): an array of `Verdict` objects (schema owned by `AION_Architecture`).
...
```
$ aion optimize packet_stats.cpp --profile perf.data
...
Runs AION automatically on every pull request, so optimization becomes part of code review instead of a separate chore.

**How it works, step by step:**
1. The PR triggers the action.
2. It runs `aion optimize --changed --base <target-branch> -p compile_commands.json --json` — scoped to just the files the PR touched.
...
- **`prevent` (blocking):** runs the **contracts as checks**. If new code re-introduces a pattern AION already fixed — e.g. a hot `push_back` loop with no `reserve` — the check **fails the build** (`--fail-on contract-violation`). This turns the optimizer into a regression *preventer*, the stickiest use.

**What the developer sees** — a bot comment on the PR:

```
🟢 AION — verified optimization in build_histogram()
...
**Tech:** a thin YAML action wrapping the CLI — no engine logic; it just runs `aion` and renders `--json`.
**Why first among the v1 surfaces:** highest leverage, zero new UI, and the surface Codeflash leads with.

```yaml
# .github/workflows/aion.yml (sketch)
- uses: aion/optimize-action@v1
  with:
    paths: ${{ changed_files }}
...
2. In the background, AION analyses it and marks hot regions with a **gutter icon**.
...
- It talks to a **local `aion` daemon** over an LSP-style JSON-RPC protocol.
...
**Config:** reads the project's `.aion.toml` (min-rung, transforms, profile source), so editor and CLI behave identically.
...
The backend for **Axis E** — a shared store of verified `(transform, contract, rung)` tuples that makes every AION instance smarter.
...
The engine, importable — for when you want AION inside your own program instead of behind a CLI.
...
- **One Ledger.** Every accept/reject from every surface is recorded once, so `aion report` and the web dashboard always agree — there's no per-surface bookkeeping to drift.
- **One config.** `.aion.toml` is the single source for profile source, min-rung, budgets, and enabled transforms — change it once and every surface follows.
...
*Surface specs live here; the Engine API they all call is specified in `AION_Architecture`; the concepts behind the work are in `AION.md`. One fact, one home.*
