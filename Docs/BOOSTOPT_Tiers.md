# BOOSTOPT — Free vs Premium, decided at the command/flag level (and *why*)

*Companion to [BOOSTOPT_Hosted](BOOSTOPT_Hosted.md) (the cloud tier) and [BOOSTOPT_Surfaces](BOOSTOPT_Surfaces.md) (the full CLI). This doc makes the tiering decision concrete — every command and flag, sorted, with the reason.*
...
> **Premium = capabilities that need BOOSTOPT's servers.**
...
There is **no `if premium:` in the open client.** The free client is fully functional on its own. A premium capability is a *service call* gated by a **`boostopt-token` + server-side entitlement** — a flag that needs the server simply *fails without a token*, it is not "present but disabled." This is what stops the "just patch the OSS client" crack, and keeps the free tier honest.

---

## 2. The test you apply to any new flag

> **"Does this run entirely on the user's machine, or does it call BOOSTOPT's servers?"**
> Local → **free**. Server → **premium** (needs a `boostopt-token`).
...
| Command | Why free |
|---|---|
| `boostopt analyze` | Inspect opportunities locally — no writes, no server. |
| `boostopt optimize` | The core loop, runs on your machine. |
| `boostopt report` | Reads your local ledger. |
| `boostopt init` | Sets up the local `.boostopt/` workspace. |
| `boostopt serve` | A warm **local** daemon — your process, your socket. |
...
→ *These point BOOSTOPT at **your** test suite / profile on **your** machine. Nothing hosted.*
...
## 4. PREMIUM — hosted add-ons (need BOOSTOPT's servers)
...
|---|---|---|
| **Managed model** — zero-setup, curated frontier model, no API key, no local GPU | `--model hosted` (+ `--boostopt-token`) | Runs on **our** compute; we pay for inference. Value = a strong model with *nothing to install or configure* (vs. the free local 7B on your CPU). |
| **Clean-room benchmarking** — deterministic, noise-free timing on dedicated hardware | `--bench remote` / hosted verdict | The durable fix for noisy cloud/CI timing (see [BOOSTOPT_Hosted](BOOSTOPT_Hosted.md) §6). Runs on **our** isolated boxes; real infra cost, materially better verdicts. |
| **Team / org layer** — shared optimization history, cross-repo baselines, org-wide policy, dashboards | new `boostopt team …` surface + web dashboard | Stateful, multi-user, hosted. Collaboration + governance value a single dev doesn't need. |
...
| **Managed-model spend** — actual billing for hosted inference | metered via `--boostopt-token` | Pass-through + margin on real inference cost. The **local** `--budget` meter stays free. |

**The unlock is one thing:** a **`boostopt-token`** (already the Action's `boostopt-token` input). No token → the hosted paths fail cleanly; everything local still works. That is the entire gating mechanism (**P4**).
...
| Flag | Ruling | Why |
|---|---|---|
| `--model frontier` | **Free** (BYO key) | You bring your *own* OpenAI-compatible endpoint + key (an env var). It never touches our servers, so it's free. It becomes premium **only** when pointed at *BOOSTOPT's managed* endpoint (that's `--model hosted`). |
...
4. **The self-run CI Action** — BOOSTOPT in *your* pipeline.
...
> **BOOSTOPT is free where it runs on your machine — the whole optimizer, the whole proof. You pay only for our servers doing what your laptop can't: a managed model, clean-room timing, and the team layer.**
