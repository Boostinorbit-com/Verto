...
Mirrors BOOSTOPT_Architecture §16.3. Uses the flags from compile_commands.json when
...
def _fast_linker(cxx: str) -> tuple[str, ...]:
    ...
    for ld in ("mold", "lld", "gold"):
        with tempfile.TemporaryDirectory(prefix="boostopt-ld-") as wd:
            src = Path(wd) / "p.cpp"
...
def _build_program(source_code: str, out_path: str, flags: list[str],
    ...
    if cc:
        ...
        srcdir = Path(tempfile.gettempdir()) / "boostopt-ccsrc"
...
def compile_program(source_code: str, out_path: str, *, flags: list[str],
    ...
    cache_dir = Path(tempfile.gettempdir()) / "boostopt-exe"
