...
Phase-1 item #1 (BOOSTOPT_Roadmap §4). The harness embeds the target TU's full
source + a driver main(), but a function that calls into OTHER translation units
leaves those symbols undefined → the link fails → BOOSTOPT can't verify it. Here we
...
CXX = "clang++"
_OBJ_DIR = Path(tempfile.gettempdir()) / "boostopt-obj"
_ARC_DIR = Path(tempfile.gettempdir()) / "boostopt-archive"
