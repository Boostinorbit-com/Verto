"""Configuration & policy — loaded from .boostopt.toml, applied across all surfaces.

Mirrors BOOSTOPT_Architecture §8 (Config). Holds the gate policy the Invariant
...
def global_config_path() -> Path:
    """Machine-wide user defaults: ``$XDG_CONFIG_HOME/boostopt/config.toml`` (default
    ``~/.config/boostopt/config.toml``). Deliberately NOT ``~/.boostopt/`` — XDG-idiomatic, and it
    never collides with the per-project ``.boostopt/`` workspace that discovery walks up to find."""
    base = os.environ.get("XDG_CONFIG_HOME") or str(Path.home() / ".config")
    return Path(base) / "boostopt" / "config.toml"
...
class Config:
    ...
    # size of BOOSTOPT's synthetic harness binary (driver+STL dominated), and it proved
    ...
    # The client holds NO premium logic; it's a thin caller (see boostopt/surfaces/hosted_client.py).
    hosted_url: str = "http://127.0.0.1:8724"   # dev default = the local boostopt_server; prod = our host
    boostopt_token: str = ""                       # entitlement key; empty → hosted paths refuse (401)
    ...
    def load(cls, path: str | Path | None = None) -> "Config":
        ...
        Precedence: project `.boostopt.toml` > global `~/.config/boostopt/config.toml` > code defaults
        (the git model — repo config overrides user config). `path` overrides the project file."""
        cfg = cls()
        if tomllib is None:
            return cfg
        project = Path(path) if path else Path(".boostopt.toml")
        for p in (global_config_path(), project):     # global is the base; project wins
            if not p.exists():
                continue
            with p.open("rb") as f:
                data = tomllib.load(f)
            for k, v in data.get("boostopt", {}).items():
                if hasattr(cfg, k):
                    setattr(cfg, k, v)
        return cfg
