...
Mirrors BOOSTOPT_Architecture §5. analyze / optimize / report. Returns structured
...
class Engine:
    def __init__(self, config: Config | None = None) -> None:
        ...
        # Ledger lives in the `.boostopt/` workspace when one exists (post `boostopt init`),
    ...
    def optimize(self, file: str, *, apply: bool, build: dict | None = None,
        ...
        # persist=True even on a DRY RUN: the ledger is the record of what BOOSTOPT tried and
        # why it said no, which is most of its value on a file that yields no win. It used
        # to key on `apply`, so `boostopt optimize` without --apply wrote every verdict to
