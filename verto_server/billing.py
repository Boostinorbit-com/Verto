...
`--verto-token`" without a human in the loop.
...
event shape are handled with stdlib, matching verto_server's stdlib-first skeleton.
"""
...
def handle_event(event: dict) -> dict:
    ...
    if etype == "checkout.session.completed":
        ...
        if not plan or plan not in entitlement.PLANS:
            raise WebhookError(400, f"no known VERTO plan for this checkout (plan={plan!r}); "
    ...
    if etype == "customer.subscription.deleted":
        # PRODUCTION needs a customer_id→token index; the stub revokes via metadata.verto_token.
        token = (obj.get("metadata") or {}).get("verto_token")
