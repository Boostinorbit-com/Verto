# verto_server — the private hosted backend (premium tier)

> ⚠ **PRIVATE. Never published. Never shipped in the free `verto` client. Never imported by `verto`.**
...
```
verto_server  ── imports ──▶  verto     (the shared, free-tier engine)
verto         ── imports ──▶  ∅          (never reaches back)
```

The dependency arrow points **one way**. `verto_server` **reuses** the free engine (gate,
...
**cannot** contain a paywall — there's no `if premium:` to sneak in. See `Docs/VERTO_Tiers.md` (P4).

## Run it

```bash
# from the repo root
python -m verto_server.app                    # → http://127.0.0.1:8724
...
| `managed_model.py` | reuse `verto.engine.Engine` on our compute | **real** — this is the one-way dependency in action |

## How the free client will call it

`--model hosted` is a **thin, dumb HTTP client** inside the open CLI: it POSTs the function +
`verto-token` and prints the returned proof. It holds **zero** premium logic — all the value
...
- **The managed model:** wire `model="frontier"` to VERTO's *own* inference endpoint (GPU box), so
  `verto-managed-coder` runs on our hardware — the whole point of the paid model.
- **Clean-room benchmarking** (`cleanroom` feature): route the verify step to dedicated, quiet
  hardware for deterministic timing (the honest-caveat upsell — `Docs/VERTO_Hosted.md` §6).
- **Packaging:** `verto_server/` must be **excluded** from the published free wheel
  (`pyproject.toml` packages `verto` only). Deploy it from its own private image.
