...
Sends the file + token to `verto_server` and turns the returned JSON into the SAME `Verdict`
...
the free client's only link to the paid service. See Docs/VERTO_Premium.md and verto_server/.
"""
...
def run(path: str, *, url: str, token: str, apply: bool = False, backup: bool = False,
    ...
    if not token:
        raise ValueError("--model hosted needs a token — pass --verto-token or set VERTO_TOKEN. "
    ...
    try:
        ...
        detail = _detail(e)
        msg = {401: f"hosted: token rejected ({detail}) — check --verto-token / VERTO_TOKEN.",
        ...
        raise ValueError(f"hosted: can't reach the server at {url} ({e.reason}). "
                         f"Is verto_server running?  (dev: python -m verto_server.app)")
