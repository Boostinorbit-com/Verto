...
Mirrors VERTO_Surfaces (server mode). `verto serve` holds a persistent process
...
probe (build.sanitizer_toolchain). `verto optimize`/`analyze` then become thin
...
def _sock_path() -> Path:
    base = Path(tempfile.gettempdir()) / f"verto-{os.getuid()}"
...
def serve(stop: bool = False) -> int:
    ...
    if stop:
        if not sp.exists():
            print("verto: no daemon running", file=sys.stderr)
            return 1
        try:
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as c:
                ...
                _recv_all(c)
            print("verto: daemon stopped")
            return 0
        except OSError:
            sp.unlink(missing_ok=True)      # stale socket
            print("verto: cleared stale daemon socket")
            return 0
    ...
    if sp.exists():
        try:
            with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as c:
                c.connect(str(sp))
            print(f"verto: daemon already running on {sp}", file=sys.stderr)
            return 2
        except OSError:
    ...
    srv.bind(str(sp))
    srv.listen(16)
    print(f"verto daemon listening on {sp}  (Ctrl-C to stop)")
    _prewarm()
    print("verto daemon warm — ready.")
