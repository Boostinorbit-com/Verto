...
When a file has several optimization candidates, VERTO must optimize the one that
...
VERTO_Surfaces / VERTO_Architecture §11 (Evidence).
"""
...
def profile_functions(source: str, funcs: list[str], *, n: int = 200_000) -> dict[str, float]:
    with tempfile.TemporaryDirectory(prefix="verto-prof-") as wd:
