#!/usr/bin/env bash
# AION — end-to-end self-check. Runs the real engine on the example C++ files.
...
for f in packet_stats map_safe map_ordered; do
  ...
  printf "\n== examples/%s.cpp ==\n" "$f"
  "$PY" -m aion.surfaces.cli optimize "examples/$f.cpp" --offline
done
echo
...
rule
"$PY" - <<'PY'
from aion.adapters.language.cpp import _ast
...
from aion.engine.models import Target, Variant
