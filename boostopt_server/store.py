"""Persistent account + usage store for boostopt_server (PREMIUM — PRIVATE, never published).
...
(`python -m boostopt_server.admin issue --plan pro`) instead of a Stripe webhook. At release the
...
(SQLite/Postgres) behind this same small surface. See boostopt_server/README.md.
"""
...
def _default_path() -> Path:
    """Where accounts live. Override with $BOOSTOPT_SERVER_DATA (a directory); default ~/.boostopt_server."""
    base = os.environ.get("BOOSTOPT_SERVER_DATA") or str(Path.home() / ".boostopt_server")
...
def reset_store() -> None:
    """Drop the cached singleton so the next call re-reads $BOOSTOPT_SERVER_DATA (for tests)."""
    global _STORE
    _STORE = None
