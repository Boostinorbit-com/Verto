"""boostopt_server HTTP API — the hosted endpoint the free client's `--model hosted` calls.

Stdlib `http.server` (no new deps — matches BOOSTOPT's stdlib-first ethos; production would swap in
FastAPI/uvicorn behind a load balancer). Run:  python -m boostopt_server.app

  GET  /healthz        → liveness
  POST /v1/optimize    → {source, filename?}  (Authorization: Bearer <boostopt-token>)
...
class _Handler(BaseHTTPRequestHandler):
    def do_GET(self):
        if self.path == "/healthz":
            return self._json(200, {"ok": True, "service": "boostopt_server"})
    ...
    def do_POST(self):
        ...
        if self.path == "/v1/webhooks/stripe":                 # billing: gated by Stripe SIGNATURE,
            return self._stripe_webhook()                      # NOT a boostopt-token — so it's handled first
        ...
        if ent is None:
            return self._json(401, {"error": "invalid or missing boostopt-token"})
    ...
    def _token(self) -> str:
        ...
        return h[7:].strip() if h.startswith("Bearer ") else self.headers.get("X-Boostopt-Token", "")
...
def serve(host: str | None = None, port: int | None = None) -> None:
    try:                                                        # line-buffer stdout so logs appear
        ...
        pass
    host = host or os.environ.get("BOOSTOPT_SERVER_HOST", "127.0.0.1")
    port = int(port or os.environ.get("BOOSTOPT_SERVER_PORT", 8724))
    srv = None
    for attempt in (1, 2):
        try:
            ...
            if attempt == 1:                                     # busy → take over the SAME port
                print(f"boostopt_server: port {port} busy — freeing it and taking over (dev)…",
                      flush=True)
                _free_port(port)
                time.sleep(1)
                continue
            print(f"boostopt_server: still can't bind {host}:{port} ({e}). "
                  f"Free it manually:  fuser -k {port}/tcp", file=sys.stderr)
            raise SystemExit(1)
    print("boostopt_server (PRIVATE — never publish)")
...
if __name__ == "__main__":
    # `python -m boostopt_server.app` | `... 8725` (port) | `... 0.0.0.0 8725` (host port)
    _a = sys.argv[1:]
