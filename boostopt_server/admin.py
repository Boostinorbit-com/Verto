"""boostopt_server admin CLI (PREMIUM — PRIVATE). Manage tokens + see usage.
...
server reads ($BOOSTOPT_SERVER_DATA, default ~/.boostopt_server).

  python -m boostopt_server.admin plans                       # what plans exist + what they grant
  python -m boostopt_server.admin list                        # every token, its plan + usage
  python -m boostopt_server.admin issue --plan pro --note x    # mint a token (prints the secret)
  python -m boostopt_server.admin usage <token>               # runs used this period
  python -m boostopt_server.admin revoke <token>              # deactivate a token
"""
...
def _cmd_list(_a) -> int:
    ...
    if not accts:
        print("(no accounts — issue one:  python -m boostopt_server.admin issue --plan pro)")
        return 0
...
def _cmd_issue(a) -> int:
    ...
    print(f"  {acct.token}")
    print("give this to the customer as their --boostopt-token (BOOSTOPT_TOKEN).")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    p = argparse.ArgumentParser(prog="boostopt_server.admin", description="Manage hosted tokens.")
